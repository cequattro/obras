...
import pe.gob.sunafil.gestionpersonal.mybatis.despachadores.RptNoMarcacionDespatch;
...
@ManagedBean(name = "reporteNoMarcacionBean")
@SessionScoped
public class ReporteNoMarcacionBean {
	...
	private List<Map> listaReporteNoMarcacion;
	...
	public List<Map> getListaReporteNoMarcacion() {
		return listaReporteNoMarcacion;
	}

	public void setListaReporteNoMarcacion(List<Map> listaReporteNoMarcacion) {
		this.listaReporteNoMarcacion = listaReporteNoMarcacion;
	}
	...
	public String verRptNoMarcacion() {
		intendenciaSeleccionada="";
		dependenciaParaAutocompletar = "";
		dependenciaSeleccionada = null;
		listaReporteNoMarcacion=null;
		filtro = new VstPersonalrrhh();
		mes="";
		anio="";
		String vista = "pretty:rptNoMarcacionPretty";
		// CODIGO
		return vista;
	}
	...
	public StreamedContent rptNoMarcacion(long dat)
	{
		//valida
		...
		try {
			...
			RptNoMarcacionDespatch reporteDespatch=new RptNoMarcacionDespatch();
			...
			listaReporteNoMarcacion=reporteDespatch.reporteNoMarcacion(parametros);
			
			if(listaReporteNoMarcacion!=null)
			{
				List<String> intendenciasList=new ArrayList<String>();
				Workbook wb = new XSSFWorkbook();
			    Sheet sheet = wb.createSheet("REPORTE DE NO MARCACIONES");
			    ...
			    cell.setCellValue("REPORTE DE NO MARCACIONES ");
	    	    ...
	    	    cell.setCellValue("TIPO MARCACIÓN FALTANTE");
	    	    cell.setCellStyle(style1);
	    	    ...
	    	    cell.setCellStyle(style1);
	    	    for (Iterator iterator = listaReporteNoMarcacion.iterator(); iterator
						.hasNext();) {
		    	    ...
		    	    row.createCell(2).setCellValue(map.get("TIPO_FALTANTE")+"");
	    	    ...
	    	    fileOut = new FileOutputStream(rutaRpt+"/rptNoMarcacion"+rnd+".xlsx");
				wb.write(fileOut);
				fileOut.close();
				InputStream stream;
				try {
					stream = new FileInputStream(rutaRpt+"/rptNoMarcacion"+rnd+".xlsx");					
					StreamedContent fileDownload = new DefaultStreamedContent(stream, "application/vnd.ms-excel", "rptNoMarcacion-"+rnd+".xlsx");
					return fileDownload;
				} catch (Exception e) {
