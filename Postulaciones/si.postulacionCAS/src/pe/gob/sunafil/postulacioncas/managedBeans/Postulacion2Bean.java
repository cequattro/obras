...
private static Logger logger = Logger.getLogger(Postulacion2Bean.class.getName());
	...
	HashMap item_a_eliminar;
	
	
	public HashMap getItem_a_eliminar() {
		return item_a_eliminar;
	}

	public void setItem_a_eliminar(HashMap item_a_eliminar) {
		this.item_a_eliminar = item_a_eliminar;
	}
	...
	public void avisoRemoveItemFormacion(HashMap delItem)
	{
		item_a_eliminar= delItem;
		RequestContext.getCurrentInstance().execute("dialogEliminarItemFormacion.show()");
	}
	
	public void removeItemFormacion()
	{
		if(lstFormacion!=null)
		{
			lstFormacion.remove(item_a_eliminar);
	...
	public boolean validaAddItemFormacion()
	{
		...
		if(fe!=null){
			if(!fe.before(new Date())){
				Funciones.mostrarMensajeError("La fecha de egreso no puede ser posterior a la fecha actual.");				
				return false;
			}
			if(fe.before(fi)){
				Funciones.mostrarMensajeError("La fecha de egreso no puede ser anterior a la fecha de inicio.");				
				return false;
			}			
		}
		else
		...
		if(fex!=null){
			//fez>fe
			if(!fex.before(new Date())){
				Funciones.mostrarMensajeError("La fecha de extensión de diploma no puede ser posterior a la fecha actual.");				
				return false;
			}
			if(fex.before(fe)){
				Funciones.mostrarMensajeError("La fecha de extensión de diploma no puede ser anterior a la fecha de egreso.");				
				return false;
			}		
		}
		/*else
	...
	public void avisoRemoveItemCapacitacion(HashMap delItem)
	{
		item_a_eliminar= delItem;
		RequestContext.getCurrentInstance().execute("dialogEliminarItemCapacitacion.show()");
	}
	
	public void removeItemCapacitacion()
	{
		if(lstCapacitacion!=null)
		{
			lstCapacitacion.remove(item_a_eliminar);
	...
	public void avisoRemoveItemOtrosDoc(HashMap delItem)
	{
		item_a_eliminar= delItem;
		RequestContext.getCurrentInstance().execute("dialogEliminarItemOtrosDocs.show()");
	}
	
	public void removeItemOtrosDoc()
	{
		if(lstOtrosDoc!=null)
		{
			lstOtrosDoc.remove(item_a_eliminar);
	...
	public String datos1Postulaciones(){
