...
public class CombosBean implements Serializable {
	...
	private List<Combo> listaDepartamentosEmp;
	...
	/**
	 * Devuelve la lista de departamenteos (todos los departamentos)
	 * @return
	 */
	...
	/**
	 * Devuelve la lista de departamentos en los cuales SUNAFIL tiene Competencia para las empresas que se denuncien
	 * @return
	 */
	public List<Combo> getListaDepartamentosEmp() {
		
		try{
			if(listaDepartamentosEmp==null){				
				listaDepartamentosEmp=new ArrayList<Combo>();
				dspcombos=new CombosDespatch();
				listaDepartamentosEmp=dspcombos.listaComboXTipo("DepartamentoEmp");
			}
		} catch (Exception e) {
			logger.error("Error en Listar los DepartemanetosEmp: "+e.getMessage(),e);
		}
		return listaDepartamentosEmp;
	}
