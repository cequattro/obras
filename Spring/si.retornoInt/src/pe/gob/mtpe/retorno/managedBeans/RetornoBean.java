...
import pe.gob.mtpe.retorno.bean.Correo;
...
import pe.gob.mtpe.retorno.prime.RetornoDataModel;
...
import pe.gob.mtpe.retorno.utils.MailUtil;
...
public class RetornoBean implements Serializable {
	...
	final static Logger logger = Logger.getLogger(RetornoBean.class);
	...
	private List<Mensaje> lstPreguntasUsu;
	private Mensaje mensaje;
	private List<Map> lstRetornoRes;
	...
	private ParamAdmRetorno paramAdmRetorno;
	private Correo correo;

	private RetornoDataModel lstRetorno;
	private List<Map> lstRetornoSel;

	private List<Map> lstReporte;
	...
	private void inicializarObjetos() {
		...
		nlenthNumdoc = 0;
		paramAdmRetorno = new ParamAdmRetorno();
		lstRetorno = new RetornoDataModel();
		lstRetornoSel = new ArrayList<Map>();
		correo = new Correo();
		lstPreguntasUsu = new ArrayList<Mensaje>();
		mensaje = new Mensaje();
		lstRetornoRes = new ArrayList<Map>();
		txtInsertarPersona="";
	...
	public void inicializarListas() throws Exception {
		// comunService=new ComunServiceImpl();
	...
	public void enviarCorreoRespuesta() {
		String contenido = mensaje.getvTitmensaj() + ": "
				+ mensaje.getvRespta();
		String asunto = "RESPUESTA PLATAFORMA DE RETORNO PRODUCTIVO";
		String email = "";
		String mensajeCor = "";
		try {
			mensaje.setvCodusu("MZUMAETA");
			mensaje.setvEstreg("R");
			mensaje.setdFecrpta(new Date());
			mensajeService.actualizarMensaje(mensaje);

			String ruta = Funciones.ObtieneProperty(
					"pe.gob.mtpe.retorno.util.properties.mtpe_app",
					"mtpe.framework.ruta");
			String url = Funciones.leerPropertyExterno(ruta,"mtpe.framework.url");
			url += "/pasePregunta";

			for (int i = 0; i < lstRetornoRes.size(); i++) {
				if (mensaje.getvCodtdocide().equals(
						((Map) lstRetornoRes.get(i)).get("V_CODTDOCIDE")
								.toString().trim())
						&& mensaje.getvNumdoc().equals(
								((Map) lstRetornoRes.get(i)).get("VNUMDOC")
										.toString().trim())) {
					email = ((Map) lstRetornoRes.get(i)).get("VEMAIL")
							.toString().trim();
					mensajeCor = MailUtil.getMessageFormat(
							lstRetornoRes.get(i), contenido, url);
					javaMailService.enviarNormal(asunto, mensajeCor, email);
				}
	...
	public void mostrarFormRespuesta() {
		try {
			mensaje = mensajeService.morstarPregunta(mensaje);
		} catch (Exception e) {
			logger.error(e);
		}
	}
	...
	public void mostrarPreguntas() {
		try {
			lstPreguntasUsu = mensajeService.listaPreguntas(dpersona);
		} catch (Exception e) {
			logger.error(e);
		}
	...
	public void mostrarFichaPDF() throws Exception {

		Map parameters = new HashMap();
		List<Map> lista = new ArrayList<Map>();
		lista = retornoService.mostrarFicha(dpersona);
		String reporte = "ficha_retorno.jrxml";
		String nombreReporte = "ficha_retorno";
	...
	public void enviarCorreo() throws Exception {
		...
		String email = "";
		String mensajeCor = "";
		String ruta = Funciones.ObtieneProperty("pe.gob.mtpe.retorno.util.properties.mtpe_app","mtpe.framework.ruta");
		String url = Funciones.leerPropertyExterno(ruta, "mtpe.framework.url");
		url += "/pasePregunta";
		for (int i = 0; i < lstRetornoSel.size(); i++) {
			email = ((Map) lstRetornoSel.get(i)).get("VEMAIL").toString().trim();
			mensajeCor = MailUtil.getMessageFormat(lstRetornoSel.get(i),contenido, url);
			javaMailService.enviarNormal(asunto, mensajeCor, email);
	...
	public void listarRetorno() throws Exception {
		lstRetornoRes = retornoService.listarAdmRetorno(paramAdmRetorno);
		lstRetorno = new RetornoDataModel(lstRetornoRes);
	}

	public String mostrarFormulario() throws Exception {
		String vista = "pretty:fichaRetorno";
	...
	public ParamAdmRetorno getParamAdmRetorno() {
		return paramAdmRetorno;
	}

	public void setParamAdmRetorno(ParamAdmRetorno paramAdmRetorno) {
		this.paramAdmRetorno = paramAdmRetorno;
	}
	...
	public RetornoDataModel getLstRetorno() {
		return lstRetorno;
	}

	public void setLstRetorno(RetornoDataModel lstRetorno) {
		this.lstRetorno = lstRetorno;
	}

	public List<Map> getLstRetornoSel() {
		return lstRetornoSel;
	}

	public void setLstRetornoSel(List<Map> lstRetornoSel) {
		this.lstRetornoSel = lstRetornoSel;
	}

	public List<Map> getLstReporte() {
		return lstReporte;
	}

	public void setLstReporte(List<Map> lstReporte) {
		this.lstReporte = lstReporte;
	}

	public List<Mensaje> getLstPreguntasUsu() {
		return lstPreguntasUsu;
	}

	public void setLstPreguntasUsu(List<Mensaje> lstPreguntasUsu) {
